...
/obj/item/mecha_parts/mecha_equipment/melee_weapon/sword/cleave_attack()	//use this for basic cleaving attacks, tweak as needed
	...
	for(var/i = 0 to 2)
		...
		for(var/atom/A in T.contents)
			...
			else if(isstructure(A) || ismachinery(A) || istype(A, /obj/vehicle/sealed/mecha))	//if it's something we can otherwise still hit
				var/obj/O = A
...
/obj/item/mecha_parts/mecha_equipment/melee_weapon/sword/precise_attack(atom/target)
	...
	else if(isstructure(target) || ismachinery(target) || istype(target, /obj/vehicle/sealed/mecha))	//If the initial target is a big object, hit it even if it's not dense.
		var/obj/O = target
